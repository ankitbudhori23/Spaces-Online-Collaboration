...
var io = require("socket.io")(server, {
  cors: {
    origin: [
      "https://vasu-mandhanya.github.io:*",
      "https://siddharth9901.github.io:*",
    ],
    methods: ["GET", "POST"],
  },
});
  ...
  socket.on("disconnect", () => {
    ...
    try {
      ...
      if (room) {
        ...
        const newUsers = RoomUsers[roomID].filter(
          (id) => id.socket !== socket.id
        );
  ...
  socket.on("join room", (data) => {
    ...
    try {
      if (VerifyToken(data.token)) {
        ...
        const usersInThisRoom = RoomUsers[data.roomID].filter(
          (id) => id.socket !== socket.id
        );
