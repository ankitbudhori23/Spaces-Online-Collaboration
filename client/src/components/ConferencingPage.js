...
class ConferencingPage extends Route {
  ...
  async getData() {
    ...
    const getSpaceDataURL =
      "http://localhost:5000/getMyConversation/" + `${token}`;
    axios
      .get(`${getSpaceDataURL}`)
      .then((res) => {
    ...
    const getMembersDataURL =
      "http://localhost:5000/viewContacts/" + `${token}`;
    axios
      .get(`${getMembersDataURL}`)
      .then((res) => {
        console.log(res.data);
        this.setState({ members: res.data, loadingMembers: false });
        console.log("fddd", this.state.members);
        this.setState({
          name: this.state.members.find(
            (member) => member.email == jwt.decode(token).username
          )?.name,
        });
      })
      .catch((err) => {
    ...
    //GETING Admin DATA:--->
    const getAdminDataURL = "http://localhost:5000/viewAdmin/" + `${token}`;
    axios
      .get(`${getAdminDataURL}`)
      .then((res) => {
        console.log(res.data);
    ...
    //     const getSpaceDataURL = "http://localhost:5000/profileInfo/" + `${email}`;
    ...
    //this.ProfileEmail = email;
    const getProfileDataURL = "http://localhost:5000/profileInfo/" + `${token}`;
    axios
      .get(`${getProfileDataURL}`)
      .then((res) => {
        console.log(res.data);
        this.setState({ user: res.data, loading: false });
      })
  ...
  async componentWillMount() {
    ...
    this.state.socket.on("Call Accepted", (e) => {
      ...
      if (history)
        history.push(
          `/Room/${id}/${this.state.videocall}/${this.state.user.name}`
        );
    });
  }
  ...
  getCurrentChat = async (recEmail) => {
    ...
    const getCurrentChatID =
      "http://localhost:5000/getCurrentChatID/" +
  ...
  getMessages = async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        const getMessages =
          "http://localhost:5000/getMessages/" + `${this.state.currentChatID}`;
        axios
          .get(`${getMessages}`)
          .then((res) => {
            console.log(res.data);
        ...
        //const res=axios.get("http://localhost:5000/getMessages/"+this.state.currentChatID)
        //GETING currentChatID:--->
      })
      .catch((err) => {
        console.log(err);
  ...
  handleNewMessage() {
    ...
    if (this.state.message !== "") {
      ...
      if (this.state.currentChat != "Group") {
        var message = {
          sender: jwt.decode(localStorage.getItem("token")).username,
          text: this.text,
          createdAt: Date.now(),
        };
        Messages.push(message);
  ...
  createaudio = (props) => {
    const name = this.state.members.find(
          ...
          <div class="wrapper">
            ...
            <div class="main_content">
              ...
              <div style={{ position: "relative", height: "98%" }}>
                <MainContainer>
                  <ConversationList>
                    ...
                    >
                      <Avatar
                        src={"http://ui-avatars.com/api/?name=Team"}
                        name="Team"
                      />
                    </Conversation>
                    ...
                    {this.state.members.map((e) => (
                      ...
                      >
                        <Avatar
                          src={
                            "http://ui-avatars.com/api/?name=" + String(e.name)
                          }
                          name={e.name}
                          status={
                            this.state.OnlineUsers.includes(e.email)
                              ? "available"
                              : "unavailable"
                          }
                        />
                  ...
                  <ChatContainer>
                    <ConversationHeader>
                      ...
                      <Avatar
                        src={
                          "http://ui-avatars.com/api/?name=" +
                          String(
                            this.state.currentChatName != "Group"
                              ? this.state.currentChatName
                              : "Team"
                          )
                        }
                        name={this.state.currentChatName}
                      />
              ...
              <div
                ...
                id="call_alert_dialog_box"
                style={this.state.callDisplay}
              >
                <div class="common-dialog-box">
                  ...
                  <p>
                    <b>
                      {
                        this.state.members.find(
                          (member) => member.email === this.state.callerID
                        )?.name
                      }
                    </b>
                  </p>
                  ...
                  <div class="common-dialog-box-buttons-row">
                    <button
                      ...
                      onClick={() => {
                        this.state.socket.emit("Reject Call", {
                    ...
                    <button
                      ...
                      onClick={() => {
                        this.state.currentChat = this.getCurrentChat(
                          this.state.callerID
                        );
                        if (this.state.callType == "Video Call") {
                          this.state.socket.emit("Accept Call", {
                            receiverId: this.state.callerID,
                          });
                          const id = this.state.currentChatID;
                          const { history } = this.props;
                          if (history)
                            history.push(
                              `/Room/${id}/true/${this.state.user.name}`
                            );
                        } else {
                          this.state.socket.emit("Accept Call", {
                            receiverId: this.state.callerID,
                          ...
                          if (history)
                            history.push(
                              `/Room/${id}/false/${this.state.user.name}`
                            );
                        }
                      }}
                    >
              ...
              <div
                class="common-dialog-box-background"
                id="call_outgoing_alert_dialog_box"
                style={this.state.outgoingDisplay}
              >
                <div class="common-dialog-box">
                  <div class="common-dialog-box-title">
                    <h1>Outgoing Call </h1>
